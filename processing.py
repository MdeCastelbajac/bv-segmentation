...
def closed_mask(bb, tile_s, tile_g, t=100):
    ...
    for i in range(2):
      ...
      im = morphology.binary_dilation(im, morphology.disk(2))
    im = morphology.area_opening(im, 750)    
    label_im = measure.label(im)
...
def open_mask(bb, tile_g):
    ...
    tile_g_ = morphology.area_closing(tile_g_, 2000)
    ...
    reconstructed = morphology.binary_dilation(reconstructed, morphology.disk(2))

    out =morphology.area_opening(reconstructed, 10000)
...
def find_opened_vessels(bbs, g, tile_size=256):
    ...
    for bb in tqdm(bbs):
        ...
        mask, regions = open_mask(bb, tile)
        final_mask[new_bb[0]:new_bb[2], new_bb[1]:new_bb[3]] = mask
        new_bbs.append(new_bb)
        masks.append(mask)
    new_bbs, masks = clear_contours(new_bbs, masks)
